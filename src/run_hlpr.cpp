...
void run(const int (&nps)[n_dims], const user_params_t &user_params)
{
  ...
  else if (user_params.model_case == "cold_thermal")
    case_ptr.reset(new cases::cold_thermal::ColdThermal<case_ct_params_t, n_dims>(user_params.X, user_params.Y, user_params.Z));
  else
